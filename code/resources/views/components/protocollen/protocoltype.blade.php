<div class="flex justify-center content-start flex-wrap h-screen pt-14">
   ...
   $details = DB::table('protocoldetail')->where("protocoltypeid",$id)->get();
      foreach ($details as $v) {
         ?>
         <a href="/protocolinfo?id={{$v->id}}&t={{$v->name}}&c={{$color}}" class="my-5 mx-3 w-40 flex justify-between h-48 items-center flex-col">
            ...
            <h3 class="text-xl text-center font-bold">{{$v->name}}</h3>
         </a>
         <?php 
      };
   ?>
